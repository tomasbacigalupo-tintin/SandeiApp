...
export default function Players() {
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        toast({ title: 'Error al eliminar jugador', description: (e as Error).message, variant: 'destructive' });
      }
    },
    [deletePlayerMutation, toast],
  );
  ...
  const handleComplete = async (data: PlayerWizardData) => {
    try {
      ...
      toast({ title: 'Error al guardar jugador', description: (e as Error).message, variant: 'destructive' });
    } finally {
      closeModal();
    }
  };
  ...
  if (error) {
    return <p className="text-red-500 text-center mt-10">{(error as Error).message}</p>;
  }
